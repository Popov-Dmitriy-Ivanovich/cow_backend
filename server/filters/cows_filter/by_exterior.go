...
func (f ByExterior) Apply(fm filters.FilteredModel) error {
	...
	if bodyData.ExteriorFrom != nil && bodyData.ExteriorTo != nil {
		...
		query = query.Where("EXISTS(SELECT 1 FROM exteriors WHERE exteriors.cow_id = cows.id AND exteriors.rating BETWEEN ? AND ?)", bodyData.ExteriorFrom, bodyData.ExteriorTo)
		...
		query = query.Where("EXISTS(SELECT 1 FROM exteriors WHERE exteriors.cow_id = cows.id AND exteriors.rating >= ?)", bodyData.ExteriorFrom)
		...
		query = query.Where("EXISTS(SELECT 1 FROM exteriors WHERE exteriors.cow_id = cows.id AND exteriors.rating <= ?)", bodyData.ExteriorTo)
	}
	fm.SetQuery(query)
	return nil
}
