...
func (f ByInsemenationDate) Apply(fm filters.FilteredModel) error {
	...
	if bodyData.InseminationDateFrom != nil && bodyData.InseminationDateTo != nil {
		...
		query = query.Where("EXISTS (SELECT 1 FROM lactations WHERE lactations.cow_id = cows.id AND lactations.insemenation_date BETWEEN ? and ?)", bdFrom.UTC(), bdTo.AddDate(0, 0, 1).UTC())
		...
		query = query.Where("EXISTS (SELECT 1 FROM lactations WHERE lactations.cow_id = cows.id AND lactations.insemenation_date >= ?)", bdFrom.UTC())
		...
		query = query.Where("EXISTS (SELECT 1 FROM lactations WHERE lactations.cow_id = cows.id AND lactations.insemenation_date <= ?)", bdTo.AddDate(0, 0, 1).UTC())
	}
	fm.SetQuery(query)
	return nil
}
