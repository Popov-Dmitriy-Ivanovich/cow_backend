...
func (f ByAnyIllneses) Apply(fm filters.FilteredModel) error {
	...
	if bodyData.HasAnyIllnes != nil && *bodyData.HasAnyIllnes {
		query = query.Where("EXISTS (SELECT 1 FROM genetics where genetics.cow_id = cows.id" +
			" AND EXISTS (SELECT 1 FROM genetic_illness_data WHERE genetic_illness_data.genetic_id = genetics.id AND " +
			" EXISTS (SELECT 1 FROM genetic_illness_statuses where genetic_illness_statuses.id = genetic_illness_data.status_id AND  genetic_illness_statuses.status IS NOT NULL " +
			" AND genetic_illness_statuses.status <> 'FREE')))").
	...
	if bodyData.HasAnyIllnes != nil && !*bodyData.HasAnyIllnes {
		query = query.Where("EXISTS (SELECT 1 FROM genetics where genetics.cow_id = cows.id" +
			"AND NOT EXISTS (SELECT 1 FROM genetic_illness_data WHERE genetic_illness_data.genetic_id = genetics.id AND " +
