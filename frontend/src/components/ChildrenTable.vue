    ...
    <div class="parent-table">
        ...
        <div v-if="!isSearch&!search_error">
            <div v-for="animal in animals" :key="animal[0]">
                <ConcretAnimalLine v-bind:animal_item="animal" v-bind:filters="filters"/>
            </div>
        </div>
        <div v-if="isSearch&!search_error">
            <div v-for="search_animal in search_result" :key="search_animal[0]">
                <ConcretAnimalLine v-bind:animal_item="search_animal" v-bind:filters="filters"/>
            </div>
        </div>
...
<script>
import ConcretAnimalLine from './ConcretAnimalLine.vue';
import NumberPages from './NumberPages.vue';

export default {
    components: {
        ConcretAnimalLine, NumberPages
    },
    data () {
        return {
            animals: [],
    ...
    async mounted() {
        ...
        this.$emit('defPages', search_params.npage, Math.ceil(res_animals.N/50));
    },
    watch: {
        async cp(newValue) {
